...
imageM <- matrix( M[1,2:ncol(M)],nrow = imageSize,ncol = imageSize,byrow = FALSE)
...
#see one eigenvector
image(matrix(M_PCA$rotation[1:nrow(M_PCA$rotation),1],nrow = imageSize,ncol = imageSize,byrow = FALSE))



#getting the reconstruction of the images
trunc <- M_PCA$x[1,1:nrow(M_PCA$rotation)] %*% t(M_PCA$rotation[,1:nrow(M_PCA$rotation)])
